-- Program: pg2html.gc.sql
-- Info:    PostgreSQL Google Cloud SQL report in HTML
--          Works with PostgreSQL 10 or sup. (tested and updated up to 14.x)
-- Date:    2008-08-15
-- Version: 1.0.23f on 2022-11-10
-- Author:  Bartolomeo Bogliolo (meo) mail AT meo.bogliolo.name
-- Usage:   psql [-U USERNAME] [DBNAME] < pg2html.gc.sql > /dev/null
...
--          1.0.21 Latest versions update, fork for version 13
--          1.0.22 Dynamic version: should work for all PG supported releases and with more details for created extensions, casting to bigint on some statistics

--          1.0.22 Forked pg2html.gc.sql for Google Cloud SQL
...
select '<!doctype html><html><head><meta charset="UTF-8"><link rel="stylesheet" href="ux3.css" /><title>pg2html.gc - Cloud SQL for PostgreSQL Statistics</title></head><body>' as info;
select '<h1 align=center>Cloud SQL for PostgreSQL - '||current_database()||'</h1>' as info;
...
select '<li><A HREF="#par">Parameters / Database Flags</A></li>' as info;
select '<li><A HREF="#opt">Optional informations</A></li>' as info;
...
select 'using: <I><b>pg2html.gc.sql</b> v.1.0.23f' as info;
...
SELECT '<td>', CASE WHEN trunc(cast(current_setting('server_version_num') as integer)/100) in (1000, 1100, 1200, 1300, 1400, 1500) THEN 'YES'
                    ELSE 'NO'
               END; -- supported
SELECT '<td>', CASE WHEN trunc(cast(current_setting('server_version_num')as integer)/100) in (1300, 1400, 1500) THEN 'YES'
                    ELSE 'NO'
               END; -- last2 release
SELECT '<td>', CASE WHEN cast(current_setting('server_version_num')as integer)
       in (90623,90624, 100023,100022, 110019,110018,110017, 120014,120013,120012, 130010,130009,130008, 140007,140006,140005, 150000,150001,150002) THEN 'YES'
                    ELSE 'NO'
               END; -- last2 update
select '<td>Latest Releases: 15.1, 14.6, 13.9, 12.13, 11.18';
select '    <br>Latest Unsupported: 10.23, 9.6.24, 9.5.25, 9.4.26, 9.3.25, 9.2.24, 9.1.24, 9.0.23, 8.4.21, 8.3.23, 8.2.23, 8.1.23, 8.0.26, 7.4.30; 6.5.3, 1.0.9';
select '</table><p><hr>';
...
as info;
select '<tr><td>'||rolname,
 '<td align="right">'||to_char(sum(case when relkind='r' THEN 1 ELSE 0 end),'999G999G999G999G999'),
 '<td align="right">'||to_char(sum(case when relkind='r' THEN reltuples ELSE 0 end),'999G999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(case when relkind='r' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(case when relkind='i' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(case when relkind='t' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(cast(1 as bigint)* relpages *8)),'999G999G999G999G999')
...
as info;
select
 '<tr><td align="right">'||to_char(count(*),'999G999G999G999G999') obj,
 '<td align="right">'||to_char(sum(reltuples),'999G999G999G999G999') rowcount,
 '<td align="right">'||to_char(trunc(sum(relpages *8)),'999G999G999G999G999') relpages,
 '<td align="right">'||to_char(trunc(sum(pg_total_relation_size(oid))/1024),'999G999G999G999G999') total,
 '<td align="right">'||to_char(trunc(sum(pg_relation_size(oid, 'main'))/1024),'999G999G999G999G999') main,
 '<td align="right">'||to_char(trunc(sum(pg_relation_size(oid, 'fsm'))/1024),'999G999G999G999G999') fsm,
 '<td align="right">'||to_char(trunc(sum(pg_relation_size(oid, 'vm'))/1024),'999G999G999G999G999') vm,
 '<td align="right">'||to_char(trunc(sum(pg_relation_size(oid, 'init'))/1024),'999G999G999G999G999') init
from pg_class
where relkind='r';
select '</table><p>' as info;
...
SELECT '<tr><td>'||datname||'<td align="right">', age(datfrozenxid), '<td>',
       (age(datfrozenxid)::numeric/1000000000*100)::numeric(4,2) as "% Wraparound"
  FROM pg_database
 ORDER BY 2 DESC;
select '</table><p>' as info;
...
SELECT '<tr><td>'||replace(query,',',', '), ' <td>'||pg_get_userbyid(userid), '<td align="right">'||calls,
       ...
       '<td align="right">'||round(total_exec_time), '<td align="right">'||round(blk_read_time+blk_write_time), '<td align="right">'||rows,
...
select '<P><table border="2"><tr><td><b>All indexes (up to 1000)</b>' as info;
select '<tr><td><b>Schema</b><td><b>Relation</b>',
...
select '<tr><td>'||relname,
 ...
 '<td align=right>'||to_char(relpages::INT8*8*1024,'999G999G999G999G999'),
 '<td align=right>'||to_char(reltuples,'999G999G999G999G999')
...
select '<tr><td>',subname, '<td>',datname, '<td>',rolname,
       '<td>',subenabled, '<td>', subsynccommit, '<td>', subslotname, '<td>N.A.'
...
select '<tr><td>',name,'<td class="split">',replace(replace(setting,'<','&lt;'),'>','&gt;'),
   '<td align=right>',min_val,'<td align=right>',max_val,
   '<td>',unit, '<td>',short_desc, '<td>',category, '<td>',context
from pg_settings
order by name; 
select '</table><p><hr>' as info;
...
select '<P><table border="2"><tr><td><b>Bloat detailed informations for biggest tables (time expensive, uncomment if needed)</b></td></tr>';
select '<tr><td><p><pre>' as info;
...
select '<A HREF="http://meoshome.it.eu.org/">this site</A>' as info;
