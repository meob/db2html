...
--          Best with PostgreSQL 9.2 - 9.6 works with PostgreSQL between 7.0 and 9.6
-- Date:    2008-04-01
-- Version: 1.0.1 on 2021-10-31
...
--          1.0.11 pg_stat_archiver (9.4), pg_stat_activity bkw changes (9.6)
--                 (a) Schema/Function Matrix
--          1.0.12 HBA.conf file, datatypes usage
--          1.0.13 Latest versions update
--          1.0.14 Latest versions update, relkind in pg_buffercache stat, bloat stats
--          1.0.15 Latest versions update
--          1.0.16 Latest versions update
--          1.0.17 Latest versions update, (a) creation date workaround
--          1.0.18 Latest versions update
--          1.0.19 Both owner/schema in matrix, latest versions update
--          1.0.20 Latest versions update, (a) May 2021 updates, (b) August 2021 updates
--          1.0.21 Latest versions update: 9.6 desupported
...
select 'using: <I><b>pg2html.96.sql</b> v.1.0.21' as info;
select '<br>Software by ' as info;
select '<A HREF="https://meoshome.it.eu.org/#dwn">Meo</A></I><p>'
as info;
...
SELECT '<td>', CASE WHEN cast(current_setting('server_version_num')as integer)
       in (90623,90624, 100020,100018,100019, 110015,110013,110014, 120010,120008,120009, 130006,130004,130005, 140000,140001,140002) THEN 'YES'
                    ELSE 'NO'
               END; -- last2 update
select '<td>Latest Releases: 14.2, 13.6, 12.10, 11.15, 10.20';
...
select '<tr><td><b>Name</b> <td><b>Location</b>' as info;
select '<tr><td>'||spcname, ' <td>'||pg_tablespace_location(oid) from pg_tablespace;
select '</table><p><hr>' as info;
...
as info;
select '<tr><td>'||rolname,
 '<td align="right">'||to_char(sum(case when relkind='r' THEN 1 ELSE 0 end),'999G999G999G999'),
 '<td align="right">'||to_char(sum(case when relkind='r' THEN reltuples ELSE 0 end),'999G999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(case when relkind='r' THEN relpages *8 ELSE 0 end)),'999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(case when relkind='i' THEN relpages *8 ELSE 0 end)),'999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(case when relkind='t' THEN relpages *8 ELSE 0 end)),'999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(relpages *8)),'999G999G999G999G999')
from pg_class, pg_roles
where relowner=pg_roles.oid
group by rolname
order by rolname;
select '<tr><td>TOTAL',
 '<td align="right">'||to_char(sum(case when relkind='r' THEN 1 ELSE 0 end),'999G999G999G999G999G999'),
 '<td align="right">'||to_char(sum(case when relkind='r' THEN reltuples ELSE 0 end),'999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(case when relkind='r' THEN relpages *8 ELSE 0 end)),'999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(case when relkind='i' THEN relpages *8 ELSE 0 end)),'999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(case when relkind='t' THEN relpages *8 ELSE 0 end)),'999G999G999G999'),
 '<td align="right">'||to_char(trunc(sum(relpages *8)),'999G999G999G999')
from pg_class, pg_roles
where relowner=pg_roles.oid;
select '</table><p>' as info;
...
select
 ...
 '<td align="right">'||to_char(trunc(sum(pg_total_relation_size(oid))/1024),'999G999G999G999') total,
...
select '<tr><td><b>Relation</b><td><b>Index Usage%</b>', '<td><b> #Rows </b>';
SELECT '<tr><td>', relname, '<td align="right">', 100 * idx_scan / (seq_scan + idx_scan) index_used_pct,
       '<td align="right">', n_live_tup table_rows
  FROM pg_stat_user_tables
...
select '<P><pre><table border="2"><tr><td><b>Data Types</b></td></tr>'
 as info;
select '<tr><td><b>Owner</b>',
 '<td><b>Data type</b>',
 '<td><b>Count</b>'
...
select '<tr><td><b>Client</b>', '<td><b>State</b>', '<td><b>Sync</b>',
       ...
       '<td><b>xlog location</b>';
select '<tr><td>',client_addr, '<td>', state, '<td>', sync_state, '<td>', txid_current_snapshot(),
       '<td>', sent_location, '<td>', write_location, '<td>', flush_location, '<td>', replay_location,
       '<td>', backend_start , '<td>', pg_current_xlog_location()
...
select '<tr><td>', now() - pg_last_xact_replay_timestamp(),
       '<td>', CASE WHEN pg_last_xlog_receive_location() = pg_last_xlog_replay_location() THEN 0
       ...
       '<td>', pg_last_xlog_receive_location(),  
       '<td>', pg_last_xlog_replay_location();
select '</table><p>' as info;
select '<hr>' as info;
