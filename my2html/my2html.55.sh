# Program:	my2html.55.sh
# 		MySQL (5.5) DBA Database report in HTML
#
# Version:      1.0.10
...
echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8" /><link rel="stylesheet" href="ux3.css" /> <title>' $HSTN : $PRT \
   ' - my2html MySQL (5.5) Statistics</title></head><body>' > $HSTN.$PRT.htm
...
select '<li><A HREF="#stat">Performance Statistics</A> (<A HREF="#stat55">5.5</A>)</li>' ;
...
select 'using: <I><b>my2html.55.sh</b> v.1.0.10';
...
select '<tr><td>',id,
	...
	'<td>', time,
	'<td>', state,
	'<td>', substr(replace(replace(info,'<','&lt;'),'>','&gt;'),0,2048)
from processlist
where command <> 'Sleep'
order by id;
select '</table><p><hr>' ;
...
where g1.variable_name='COM_SELECT'
  and g2.variable_name='UPTIME'
union
select '<tr><!42><td>', g1.variable_name, ' #/sec.<td align=right>', format(g1.variable_value/g2.variable_value,5), '', ''
from global_status g1,global_status g2
where g1.variable_name='COMMIT'
  and g2.variable_name='UPTIME'
union
...
select '<P><A NAME="stat55"></A>' ;
select '<P><table border="2"><tr><td><b>Performance Schema Statistics (5.5)</b></td></tr>' ;
select '</table><p>';
...
select '<P><table border="2"><tr><td><b>Uptime</b><td>', round(variable_value/(3600*24),0),
...
select '<tr><td><b>Event</b>',
       '<td><b>Count Read</b>','<td><b>Count Write</b>';
select '<tr><td>',EVENT_NAME,'<td align="right">',COUNT_READ,'<td align="right">',COUNT_WRITE
  from performance_schema.file_summary_by_event_name order by COUNT_READ desc limit 10;
select '</table><p>';

select '<P><table border="2"><tr><td><b>File access</b></td></tr>' ;
select '<tr><td><b>File Name</b>','<td><b>Event Name</b>',
       '<td><b>#Read</b>','<td><b>Byte Read</b>',
       '<td><b>#Write</b>','<td><b>Byte Write</b>';
select '<tr><td>',FILE_NAME,'<td>',EVENT_NAME,'<td align="right">',
 COUNT_READ,'<td align="right">',SUM_NUMBER_OF_BYTES_READ,'<td align="right">',
 COUNT_WRITE,'<td align="right">',SUM_NUMBER_OF_BYTES_WRITE
  from performance_schema.file_summary_by_instance order by COUNT_READ desc limit 10;
select '</table><p><hr>';
